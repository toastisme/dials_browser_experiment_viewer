...
class ExperimentViewer {
    ...
    updateObservedReflections(val = null) {
        if (val) this.observedReflsCheckbox.checked = val;
        for(var i = 0; i < this.reflMeshesObs.length; i++)this.reflMeshesObs[i].visible = this.observedReflsCheckbox.checked;
    }
    updateCalculatedReflections(val = null) {
        if (val) this.calculatedReflsCheckbox.checked = val;
        for(var i = 0; i < this.reflMeshesCal.length; i++)this.reflMeshesCal[i].visible = this.calculatedReflsCheckbox.checked;
    }
    ...
    addReflectionsForPanel(panelReflections, panelData) {
        function mapPointToGlobal(point, pOrigin, fa, sa, scaleFactor = [
            1,
            1
        ]) {
        ...
        function getCrossLineMeshes(xyz, pOrigin, fa, sa, pxSize) {
            ...
            const left = mapPointToGlobal([
                xyz[0] - 1,
                xyz[1]
            ], pOrigin, fa, sa, pxSize);
            const right = mapPointToGlobal([
                xyz[0] + 1,
                xyz[1]
            ], pOrigin, fa, sa, pxSize);
            const top = mapPointToGlobal([
                xyz[0],
                xyz[1] - 1
            ], pOrigin, fa, sa, pxSize);
            const bottom = mapPointToGlobal([
                xyz[0],
                xyz[1] + 1
            ], pOrigin, fa, sa, pxSize);
            const line1 = new (0, _threeMeshline.MeshLine)();
            line1.setPoints([
                left,
                centre,
                right
            ]);
            const line2 = new (0, _threeMeshline.MeshLine)();
            line2.setPoints([
                top,
                centre,
                bottom
            ]);
            const line1Mesh = new _three.Mesh(line1, reflMaterial);
            const line2Mesh = new _three.Mesh(line2, reflMaterial);
            return [
                line1Mesh,
                line2Mesh
            ];
        ...
        const reflMaterial = new (0, _threeMeshline.MeshLineMaterial)({
            lineWidth: 1,
            color: ExperimentViewer.colors()["reflection"],
            fog: true
        });
        const bboxMaterial = new (0, _threeMeshline.MeshLineMaterial)({
            lineWidth: 1.75,
        ...
        for(var i = 0; i < panelReflections.length; i++){
            ...
            const c1 = mapPointToGlobal([
                bbox[0],
                bbox[2]
            ], pOrigin, fa, sa, pxSize);
            const c2 = mapPointToGlobal([
                bbox[1],
                bbox[2]
            ], pOrigin, fa, sa, pxSize);
            const c3 = mapPointToGlobal([
                bbox[1],
                bbox[3]
            ], pOrigin, fa, sa, pxSize);
            const c4 = mapPointToGlobal([
                bbox[0],
                bbox[3]
            ], pOrigin, fa, sa, pxSize);
            const corners = [
            ...
            const mesh = new _three.Mesh(line, bboxMaterial);
    ...
    updateGUIInfo() {
        ...
        if (this.activelyDisplayingText) return;
    ...
    resetPanelColors() {
        for(var i in this.panelMeshes)this.panelMeshes[i].material.color = this.panelColor;
    }
